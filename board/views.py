from board.models import Board, FollowBoard, StudyBoard, FollowStudyBoard
from rest_framework import viewsets, pagination,status, permissions
from board.serializers import BoardSerializer, FollowBoardSerializer, StudyBoardSerializer, FollowStudyBoardSerializer
...
class FollowStudyBoardViewSet(mixins.CreateModelMixin,
                   viewsets.GenericViewSet):
    serializer_class = FollowStudyBoardSerializer

    def get_queryset(self):
        study_board_name = self.kwargs['study_board_name']
        return FollowStudyBoard.objects.filter(study_board_id=study_board_name)

    def create(self, request, *args, **kwargs):
        # foreignkey field의 경우 primary key를 인자로 갖거나 그 dict에서 primary key field를 lookup한다.
        # 'user'의 value로 request.user.username와 request.user 모두 가능.
        study_board_name = kwargs['study_board_name']
        username = request.user.username
        # 이미 follow한 경우는 pass
        if FollowStudyBoard.objects.filter(study_board_id=study_board_name, user_id=username).exists():
            return DefaultResponse(None, username + " already followed the board named" + study_board_name, status=200)

        # follow 안 한 경우에만 follow 생성
        serializer = self.get_serializer(data={'study_board': study_board_name, 'user': username})
        serializer.is_valid(raise_exception=True)
        self.perform_create(serializer)
        return DefaultResponse(None, username + " followed the board named " + study_board_name, status=201)
