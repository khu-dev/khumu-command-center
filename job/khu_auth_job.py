...
class KhuAuthJob(BaseKhuJob):
    ...
    def process(self, data):
        user_info_html = self.login(data)
    ...
    def login(self, body_data):
        try:
            ...
            if info21_login_response.status_code != 200:
                logger.info(info21_login_response.text)

            info21_redirected_encode_response = self.sess.get("https://portal.khu.ac.kr/ksign/index.jsp")
            logger.info(info21_redirected_encode_response.status_code)
            if info21_redirected_encode_response.status_code != 200 :
                logger.info(info21_redirected_encode_response.text)
                raise Info21WrongCredential(" ID: " + body_data.get('id'))
            ...
            logger.info("Encoded Username: " + encoded_user_id)
            ...
            logger.info(info21_after_login_response.status_code)
            ...
            logger.info(user_info_response.status_code)
            if info21_after_login_response.status_code != 200:
                logger.info(user_info_response.text)
